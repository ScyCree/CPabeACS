...
class RSA_abe():
    ...
    def keygen(self, r_sets, attrs, secparam=1024):
    ...
    def encrypt(self, pk, m):
        ...
        for k,e in pk['e'].items():
            ...
            c_msk = int.from_bytes(sha256(k.encode() + hkey).digest(), byteorder='big') % pk['N']
            c[k]=((ip ** e) % pk['N']+ c_msk)%pk['N']
        return c
...
if __name__ == '__main__':
    ...
    #生成访问结构
    parser = PolicyParser()
    tree = parser.parse("(ONE and TWO) or (FOUR and THREE)")
    _dictCount = {}
